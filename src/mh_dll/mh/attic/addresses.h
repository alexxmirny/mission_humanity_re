...
#define MH_ADDRESS(address, type)				((type*)address)
#define MH_GLOBAL(address, type)				(*((type*)address))
#define MH_CALLPROC(address)					(((void(*)())address)())
#define MH_CALLFUNC(address, returnType)		(((returnType(*)())address)())
...
#define EXTERM_FUNC_DECOMPRESS_RSR_FILE_FROM_PTR				0x004f6e7e
...
static void MH_CALL_PROC_CHAR(int address, char *_eax)
{
	_asm {
		mov eax, _eax
		call [address]
...
static int MH_CALL_FUNC_INT_INT(int address, int _eax)
{
	int retValue;
	_asm {
		mov eax, _eax
...
static int MH_CALL_FUNC_INT_CHAR(int address, char* _eax)
{
	int retValue;
	_asm {
		mov eax, _eax
...
static void * MH_CALL_FUNC_VOID_CHAR(int address, char *_eax)
{
	void *retValue;
	_asm {
		mov eax, _eax
...
static void MH_CALL_FUNC_DECOMPRESS(char *srcData, char *dstPtr, int fileSize) {
  const int address = EXTERM_FUNC_DECOMPRESS_RSR_FILE_FROM_PTR;
  int localFileSize = fileSize;
  int *localFileSizePtr = &localFileSize; 
	_asm {
		push [localFileSizePtr]
		push [dstPtr]
		push [srcData]
		call [address]
    add esp, 0xc
